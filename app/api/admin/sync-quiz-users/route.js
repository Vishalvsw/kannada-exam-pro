...
export async function POST() {
  try {
    ...
    for (const result of quizResults) {
      // Extract user info from various possible field names
    ...
    // Get final count
    const finalUsers = await db.collection('users').find({ score: { $gt: 0 } }).toArray();
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully synced ${quizResults.length} quiz results`,
      totalQuizResults: quizResults.length,
      uniqueUsersExtracted: userMap.size,
      usersCreated: createdCount,
      usersUpdated: updatedCount,
      totalUsersWithScores: finalUsers.length,
      top5Users: topUsers.map(u => ({
        name: u.name,
